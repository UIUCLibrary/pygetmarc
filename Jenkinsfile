    ...
    stages {
        ...
        stage("Unit tests") {
            ...
            steps {
                node(label: "Windows") {
                            checkout scm
                            bat "${tool 'Python3.6.3_Win64'} -m tox -e pytest -- --junitxml=reports/junit-${env.NODE_NAME}-pytest.xml --junit-prefix=${env.NODE_NAME}-pytest --cov-report html:reports/coverage/ --cov=uiucprescon" //  --basetemp={envtmpdir}"
            ...
            steps {
                parallel(
                        ...
                        node(label: "Windows") {
                            ...
                            bat "venv\\Scripts\\mypy.exe -p pygetmarc --junit-xml=junit-${env.NODE_NAME}-mypy.xml --html-report reports/mypy_html"
